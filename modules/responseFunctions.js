...
function currentOrder(sessionData) {
    ...
    let total = 0;
    if (!sessionData.curOrder.length) {
        message = ["No items in your current order!"];
    } else {
        sessionData.curOrder.forEach((order) => {
            message.push(
                `${order.qty} ${order.item.name} - ${order.qty * order.item.price}`
...
function listItem(sessionData) {
    ...
    let items = JSON.parse(
        fs.readFileSync(path.join(__dirname, "../", "databank", "items.json"), "utf8")
    );
    ...
    for (startIndex; startIndex < stopIndex; startIndex++) {
        ...
        message.push(
            `select ${count}: ${items[startIndex].name}  @  GHS ${items[startIndex].price}`
        );
        count++;
    }
    ...
    if (items.length > stopIndex) {
        ...
        message.push(`for more items, enter 99`);
    }
...
function itemSelect() {
    ...
    let message = [`Please enter quantity of ${this.item.name} [1 to 9]`];
    this.addChild(0, cancel);
