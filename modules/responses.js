...
const response = (msg, sessionData) => {
    ...
    function sendMessage(sender, message) {
        let dataSender;
        if (sender == 'bot') { dataSender = "Leslie" } else { dataSender = "You"; }
        let messageData = { dataSender, message, time: moment().format('h:mm a') };
        sessionData.sessionMessages.push(messageData);
        sessionData.io.to(sessionData.userID).emit("server_says", messageData);
    }
